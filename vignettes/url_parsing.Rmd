...
The problem with this is that while you can copy it now, what happens when later you find a case it does not handle correctly?  How do you modify this mess?
```{r url_parsing_stock, eval=F}
"^(?:(?:http(?:s)?|ftp)://)(?:\\S+(?::(?:\\S)*)?@)?(?:(?:[a-z0-9\u00a1-uffff](?:-)*)*(?:[a-z0-9\u00a1-\uffff])+)(?:\\.(?:[a-z0-9\u00a1-\uffff](?:-)*)*(?:[a-z0-9\u00a1-\uffff])+)*(?:\\.(?:[a-z0-9\u00a1-\uffff]){2,})(?::(?:\\d){2,5})?(?:/(?:\\S)*)?$"
```

However with `rex` you can re-create the same regex which is much easier to later understand and modify.
```{r url_parsing_url}
...
We can then validate that it correctly identifies both good and bad URLs. (Note I removed the IP address validation from the regex and the tests)
...
This shows the power and expressiveness of building regular expressions with `rex`!
